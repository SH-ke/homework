...
void write2csv(FILE *fp_t, FILE *fp_g, double time_rowrow, double gflop);
...
int main(int argc, char ** argv)
{
    ...
    // write time_yours, gflop/time to *.csv
    FILE *fp_t = fopen("time_yours.csv", "a+");
    FILE *fp_g = fopen("gflop.csv", "a+");
    // write column_names to *.csv files
    if (fp_t != NULL || fp_g != NULL)
    {
        char *title = "size,OpenMP,OpenBlas,cuda_global,cuda_shared,cublas,cuda_yours";
        fprintf(fp_t, "\n%s\n%d,", title, m);
        fprintf(fp_g, "\n%s\n%d,", title, m);
    }else {
        printf("Write *.csv error !");
    }
...
#pragma omp parallel for
    ...
    // wirte to csv
    write2csv(fp_t, fp_g, time_value, gflop);
    ...
    write2csv(fp_t, fp_g, time_value, gflop);
    // close *.csv
    fclose(fp_t); fclose(fp_g);
    printf("size-%d write into csv successful.\n", m);
...
void write2csv(FILE *fp_t, FILE *fp_g, double time_rowrow, double gflop)
{
    // write both time and GFlops to *.csv files
    if (fp_t != NULL || fp_g != NULL)
    {
        fprintf(fp_t, "%4.5f,", time_rowrow);
        fprintf(fp_g, "%4.5f,", gflop / time_rowrow);
    }else {
        printf("Write *csv error !");
    }
}
